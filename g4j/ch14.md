...
Go 1.23 added `strings.Lines` alongside the `iter` package.
Go 1.24 extended the family with several more lazy variants:

```go
func strings.Lines(s string) iter.Seq[string]           // iterate over newline-delimited lines (Go 1.23)
...
### Wrapping `bufio.Scanner` as an iterator

`bufio.Scanner` has no built-in `iter.Seq` method, but writing the adapter takes three lines:

```go
func ScanLines(s *bufio.Scanner) iter.Seq[string] {
...
Now you can feed any `io.Reader` into a `for range` loop without the scanner machinery leaking into the call site:

```go
f, _ := os.Open("playlist.txt")
defer f.Close()

scanner := bufio.NewScanner(f)
for line := range ScanLines(scanner) {
    fmt.Println(line)
}
```

::: {.tip}
**Tip:** The `yield`-returns-`false` check in the adapter is what makes `break` and early `return` inside the `for range` loop work correctly.
Omit it and a `break` will silently drain the rest of the scanner before stopping.
:::
...
The same pattern works with a `Scanner` adapter:

```go
scanner := bufio.NewScanner(f)
next, stop := iter.Pull(ScanLines(scanner))
defer stop()

header, ok := next()        // consume the header line first
if !ok {
    return
}
processHeader(header)

for {                       // then process the data rows
    row, ok := next()
    if !ok {
        break
    }
