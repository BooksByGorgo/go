...
### `iter.Pull` --- when you need two iterators at once

`iter.Pull` converts a push-style `Seq` to a pull-style `(next, stop)` pair, which is useful when you need to step through two iterators in lockstep:

```go
next, stop := iter.Pull(ScanLines(bufio.NewScanner(r)))
defer stop()
