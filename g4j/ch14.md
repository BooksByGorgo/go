...
::: {.tip}
**Wut:** `math/rand/v2` removed the top-level `Seed` function that v1 had.
In v1 the global generator defaulted to seed `1` (so it was deterministic until you called `rand.Seed`); in v2 the global is always randomly seeded and **cannot** be pinned.
...
Seeding is a `math/rand/v2` concept only.
:::
