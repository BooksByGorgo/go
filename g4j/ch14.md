...
The `_` on `StdoutPipe()` discards an error for brevity --- in real code, check it (`StdoutPipe` fails if stdout was already set or the command already started), and check the errors from `Start`, `Run`, and `Wait` too.
...
`flag.PrintDefaults()` renders the standard flag table so you get the defaults for free without rewriting them.
...
Go 1.23 updated `Keys` and `Values` to return `iter.Seq` iterators instead of slices; `Clone` is available since Go 1.21.
