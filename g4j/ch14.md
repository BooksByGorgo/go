...
Running `./app -port 9090 -verbose` sets `*port` to `9090` and `*verbose` to `true`.
Running `./app -help` prints the usage string automatically.
...
Running `./app -help` prints something like:
...
That output lists flags, but it says nothing about positional arguments.
A tool that takes a required `<file>` argument leaves the user guessing.
