...
Three new verbs are especially useful for debugging, shown alongside `%v` from Chapter 1 for comparison:
...
\index{io package}
\index{io.Reader}\index{io.Writer}
...
`Scan()` tries to read the next line from input, and `Text()` returns the current line without advancing, so calling `Text()` twice returns the same line twice.
...
```go
var Args []string                         // command-line args; Args[0] is the program name
...
The `_` on `StdoutPipe()` discards an error for brevity --- in real code, check it (`StdoutPipe` fails if `Cmd.Stdout` was already set or if `Start` has already been called), and check the errors from both `Start` and both `Wait` calls too.
Do not be tempted to replace `c1.Start()` + `c1.Wait()` with `c1.Run()`: the `StdoutPipe` docs flatly declare `Run` incorrect on the command that owns the pipe, because `Wait` closes the pipe once the command exits.
The call order is load-bearing for the same reason: `c2.Start()` must come before `c1.Wait()` so `grep` already holds its own end of the pipe by the time `Wait` closes the parent's copy.
...
`os.Args[0]` is the path the program was invoked as, which is how the built binary prints `Usage: ./play ...`.
Under `go run` it is the path of the temporary binary the toolchain just built, so expect something gorgeous like `Usage: /tmp/go-build2057374792/b001/exe/play` until you `go build` for real.
...
Cobra is what the cool devs use: kubectl, Docker, the GitHub CLI, and Hugo are all built with it.
...
func Ext(path string) string                       // returns the extension including the dot
...
The `...` stands for the `time` and `level` keys every record carries --- trimmed here so the group stays the star of the show.
...
```go
func N[Int intType](n Int) Int           // uniform integer in [0, n)
func Float64() float64                   // uniform float in [0.0, 1.0)
func Shuffle(n int, swap func(i, j int)) // shuffle a slice
```

The `intType` constraint on `N` is unexported --- you cannot name it in your own code, so just read it as "any integer type".
...
- Always `defer ticker.Stop()` to release the ticker's runtime timer promptly.
...
3. **Calculation:** You open a 10 MB file and read it in three ways:
