...
The `log` package is Go's classic logger.
It writes to `os.Stderr` by default and prepends a timestamp automatically:

```go
log.Println("server started")              // 2024/11/22 15:04:05 server started
log.Printf("listening on :%d", port)
log.Fatal("cannot open config file")       // logs, then calls os.Exit(1)
log.Panic("unrecoverable state")           // logs, then calls panic()
```

For production services, `log` is usually replaced by a structured logger.
...
Go 1.21 added `log/slog`, a structured logger designed for machine-readable output.
The Java equivalent is SLF4J + Logback or Log4j2 with a JSON appender.

The two built-in handlers format output as text or JSON:

```go
// Text handler (logfmt-style):
...
// {"time":"2024-11-22T15:04:05Z","level":"INFO","msg":"track loaded","title":"Physical","bpm":130,"elapsed":"1.2ms"}
```

`slog.Attr` is the typed key-value pair:
...
func Any(key string, value any) Attr   // any type
```

### `slog.With` --- Adding Shared Context

\index{slog!With}

`logger.With(attrs...)` returns a new logger that includes the given attributes on every message it emits.
Use it to attach per-request or per-connection context:
...
reqLogger.Warn("rate limit approaching", slog.Int("remaining", 5))
// both lines include request_id and user automatically
```

This is the Go equivalent of SLF4J's MDC (Mapped Diagnostic Context), but without thread-local storage --- the context travels with the logger value, not the goroutine.

### `slog.Group`

\index{slog!Group}

`slog.Group` nests attributes under a named key, which JSON handlers render as a nested object:
...
// {"msg":"upload complete","file":{"name":"tracks.csv","bytes":204800}}
```
...
The `slog.Handler` interface lets you write your own output format or routing logic:

```go
type Handler interface {
    Enabled(ctx context.Context, level Level) bool       // reports whether the handler logs at level
    Handle(ctx context.Context, r Record) error          // handles a log record
    WithAttrs(attrs []Attr) Handler                      // returns handler with pre-set attributes
    WithGroup(name string) Handler                       // returns handler that groups future attributes
}
```

A common use is a handler that discards everything below a configurable level, or one that fans out to multiple sinks.
The `golang.org/x/exp/slog` package (now promoted to the standard library) ships a `slog.HandlerOptions` struct with `Level` and `AddSource` fields for the most common customizations.

::: {.tip}
**Tip:** Set the default logger once at startup with `slog.SetDefault(logger)`.
After that, package-level calls like `slog.Info(...)` use your configured handler, so library code can log without knowing which handler you chose.
:::
...
- `log/slog` (Go 1.21) provides structured, handler-based logging; use `logger.With` for per-request context.
