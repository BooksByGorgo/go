...
Go's standard library covers most of what you reach for in daily backend work --- I/O, file access, time, logging, CLI flags, and pattern matching --- without pulling in external dependencies.
...
Go 1.23 added lazy iterator variants of several `strings` functions so you avoid allocating a full `[]string` when you only need to scan forward:

```go
func strings.Lines(s string) iter.Seq[string]           // iterate over newline-delimited lines
func strings.SplitSeq(s, sep string) iter.Seq[string]   // lazy equivalent of strings.Split
func strings.FieldsSeq(s string) iter.Seq[string]       // lazy equivalent of strings.Fields
```
