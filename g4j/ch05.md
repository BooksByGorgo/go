...
func volume(x, y, w, h float64) float64 { return x*y*w*h }    // all four are float64
```
...
The difference is that Go's `init` is per-package and you can have several of them, whereas a Java static block is per-class.
