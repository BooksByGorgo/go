...
```go
func Keys[Map ~map[K]V, K comparable, V any](m Map) iter.Seq[K]    // iterator over keys (Go 1.23)
func Values[Map ~map[K]V, K comparable, V any](m Map) iter.Seq[V]  // iterator over values (Go 1.23)
```
...
```go
for i, title := range slices.All([]string{"Escape", "J'ai pas vingt ans !", "J'en ai marre !"}) {
    fmt.Printf("%d: %s\n", i, title)
}
// 0: Escape
